<script setup>
import { User, Play, List } from 'lucide-vue-next'
import { ref } from 'vue'
...
const emit = defineEmits(['play', 'open-uploader'])
...
<template>
  <div v-if="video" class="mb-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <div class="flex gap-4 p-4">
        <div class="relative flex-shrink-0">
          ...
          <span class="absolute top-1 left-1 px-2 py-0.5 text-[10px] rounded-md text-white bg-pink-500 backdrop-blur-[2px] shadow-sm font-bold">
            共 {{ video.pageCount || video.pages?.length || 1 }} 集
          </span>
        </div>
        <div class="flex-1 min-w-0 flex flex-col justify-between">
          <div>
            <h3 class="font-bold text-gray-800 line-clamp-2 text-sm leading-snug mb-1">{{ video.title }}</h3>
            <button 
              @click.stop="$emit('open-uploader', { mid: video.mid, name: video.author })"
              class="text-xs text-gray-500 flex items-center gap-1 hover:text-pink-600 transition-colors"
            >
              <User :size="12" />
              <span class="truncate max-w-[120px]">{{ video.author }}</span>
            </button>
          </div>
        ...
        <div v-if="showParsedPages" class="border-t border-gray-100">
          <div class="p-3 bg-gray-50/50">
            <div class="flex items-center justify-between mb-2 px-1">
              <span class="text-xs font-medium text-gray-500">选择分 P 播放</span>
              <span class="text-[10px] text-gray-400">共 {{ video.pages?.length || 1 }} 集</span>
            </div>
            <div class="max-h-64 overflow-y-auto space-y-1 scrollbar-thin">
              <button 
                v-for="(page, index) in video.pages" 
                :key="page.cid"
                ...
                :class="{ 'border-pink-300 bg-pink-50': currentIndex === index }"
              >
