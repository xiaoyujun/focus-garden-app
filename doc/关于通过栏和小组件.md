...
## 需求概述
- 专注计时进行时，通知栏与锁屏需持续展示任务名称与剩余时间。
- 通知支持快速操作（继续/暂停/结束），点击可返回应用。
- 锁屏/桌面小组件展示进度与快捷入口，无需打开应用即可查看。
- 首次进入应用时引导申请通知权限，若未授权需提示用户手动开启。

## 权限与系统设置
- `POST_NOTIFICATIONS`：Android 13+ 必须显式申请；未授权无法推送通知。
- `FOREGROUND_SERVICE`：用于常驻通知的前台服务，Manifest 需声明。
- 锁屏显示：通知需设置 `VISIBILITY_PUBLIC`，同时在文案中提示用户在系统“锁屏通知/显示内容”中允许显示。
- 省电/后台限制：国产 ROM 可能限制前台服务，需要在 UI 文案里提示用户将应用加入自启动/省电白名单。
...
- 锁屏可见：`setVisibility(Notification.VISIBILITY_PUBLIC)`，保证锁屏显示内容。

### 安卓端步骤（Capacitor 容器）
1. **创建前台服务**：`FocusForegroundService`，接受 `title/remainingSeconds/isRunning/progress` 构造通知。
2. **注册通知渠道**：在 `onCreate` 初始化 `NotificationChannel("focus_status")`。
3. **处理操作按键**：通过 `BroadcastReceiver` 响应暂停/结束意图，再用 `LocalBroadcast` 或自定义 Capacitor 插件回传给 WebView。
4. **Manifest 配置**：
   - 声明 `<uses-permission android:name="android.permission.POST_NOTIFICATIONS" />`
   - 注册 `service` 与 `receiver`，按需设置 `android:foregroundServiceType="dataSync|mediaPlayback"`。
...
调用时机（建议）：
- 应用进入时调用 `ensureNotificationPermission()`，未授权则弹窗引导用户开启通知/锁屏显示。
- “开始专注”调用 `startFocusNotification`，“暂停/继续”调用 `updateFocusNotification`，“结束”调用 `stopFocusNotification`。
